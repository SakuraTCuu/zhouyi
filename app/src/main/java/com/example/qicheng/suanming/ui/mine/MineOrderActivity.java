...
public class MineOrderActivity extends BaseActivity implements AdapterView.OnItemClickListener {
    ...
    protected void initView() {
        ...
        onClickOrderListener listener = new onClickOrderListener() {
            ...
            public void clickBtn(int position) {
                //点击详情页
                MineOrderBean orderBean;
                if (type == -1) { //all
                    orderBean = data.get(position);
                } else if (type == 0) {  //未支付
                    orderBean = noover_data.get(position);
                } else {  //已支付
                    orderBean = over_data.get(position);
                }
                Log.d("orderbean--->", orderBean.getOrderTitle());
